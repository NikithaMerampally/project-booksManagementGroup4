...
let data=req.body
if(Object.keys(data).length==0) return res.status(400).send({status:false,msg:"please provide fields"})
if(!data.title) {
    
    
    return res.status(400).send({status:false,msg:"please provide title"})}
    if(!["MR","MRS","MISS"].includes(title)) return res.status(400).send({status:false,msg:"title must be MR,MRS,MISS"})

if(!data.name) {
   
    
    return res.status(400).send({status:false,msg:"please provide name"})}
    if(!validator.isAlpha(data.name)) return res.status(400).send({status:false,msg:"please provide valid name"})}
if(!data.phone)
 {
...
if(!validator.isEmail(data.email))  return res.status(400).send({status:false,msg:"please provide valid email"})
...
if(checkPhone) return res.status(400).send({status:false,msg:"Phone is already in use"})
